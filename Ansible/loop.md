В Ansible есть ***циклы***. Цикл позволяет выполнить *одно и тоже с разными значениями*. Фактически, просто позволяет запускать [таску](tasks) несколько раз с разными значениями.
