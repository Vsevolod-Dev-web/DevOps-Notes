...
В loop можно сразу передать список словарей:
```yml
admins:
  - name: vsevolod
...
Тогда, так же как и выше, можно будет обращаться каждому полю списка: `"{{ item.name }}"`

`item` - это системная переменная Ansible в которой записан один элемент одной итерации. Через точку можно вызывать конкретное поле этого списка.
